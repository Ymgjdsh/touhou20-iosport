...
#include <stdexcept>
#include <emmintrin.h>
//53b5c0 core decoded in ecl_vm/vm.cpp, applied directly to the original 72-byte owning runtime.
namespace th20::source::gameplay {
using ecl::Instruction;using ecl::bits_int;using ecl::bits_float;using ecl::float_bits;using ecl::truncate_float;namespace math=ecl::math;
namespace {
...
std::uint32_t resolved_integer(EnemyRuntime& r, Instruction ins, std::int32_t index, std::uint32_t raw, bool consume) {
    if (!reference(ins,index)) return raw;
    const auto value=bits_int(raw);
    if (value >= 0) return r.stack.local(value);
    if (value < -100) return static_cast<std::uint32_t>(r.manager->read_integer(value));
    return consume ? r.stack.pop('i') : r.stack.peek(value*8,'i');
}
float resolved_float(EnemyRuntime& r, Instruction ins, std::int32_t index, float value, bool consume) {
    ...
    if (value > -1.0f || value < -100.0f) return r.manager->read_float(truncate_float(value));
    return bits_float(consume ? r.stack.pop('f') : r.stack.peek(truncate_float(mul(value,8.0f)),'f'));
}
}
std::uint32_t& EnemyRuntimeStack::absolute(std::int32_t bytes) {
...
std::uint32_t& EnemyRuntimeStack::local(std::int32_t bytes) { return absolute(frame_base+bytes); }
void EnemyRuntimeStack::push(std::uint32_t value,char type) {
...
void EnemyRuntimeStack::leave_frame() { const auto previous=frame_base;frame_base=bits_int(pop());pointer=previous; }
Instruction EnemyRuntime::current(){if(!active())throw std::out_of_range("Inactive enemy ECL runtime");return {manager->loader->instruction(subroutine,instruction_offset)};}
std::int32_t EnemyRuntime::integer_argument(Instruction ins,std::int32_t i,bool consume) { return bits_int(resolved_integer(*this,ins,i,ins.argument(i),consume)); }
float EnemyRuntime::float_argument(Instruction ins,std::int32_t i,bool consume) { return resolved_float(*this,ins,i,bits_float(ins.argument(i)),consume); }
std::int32_t EnemyRuntime::integer_value(Instruction ins,std::int32_t i,std::uint32_t bits) {return bits_int(resolved_integer(*this,ins,i,bits,false));}
float EnemyRuntime::float_value(Instruction ins,std::int32_t i,float value) {return resolved_float(*this,ins,i,value,false);}
std::uint32_t& EnemyRuntime::integer_destination(Instruction ins,std::int32_t i) {
    if(!reference(ins,i))throw std::runtime_error("Original ECL integer destination is null");
    const auto raw=bits_int(ins.argument(i));return raw<0?*manager->integer_destination(raw):stack.local(raw);
}
std::uint32_t& EnemyRuntime::float_destination(Instruction ins,std::int32_t i) {
    if(!reference(ins,i))throw std::runtime_error("Original ECL float destination is null");
    const auto raw=bits_float(ins.argument(i));return raw<0?*manager->float_destination(truncate_float(raw)):stack.local(truncate_float(raw));
}
bool EnemyRuntime::call(Instruction ins) {
    return call_into(ins,*this,0);
}
bool EnemyRuntime::call_into(Instruction ins,EnemyRuntime& target,std::int32_t argument_skip) {
    ...
    auto* previous=manager->current;manager->current=&target;target.subroutine=manager->loader->find(name);target.instruction_offset=0;target.time=0;
    if(target.subroutine<0){subroutine=-1;instruction_offset=-1;return false;}manager->current=previous;return true;
}
void EnemyRuntime::tick_interpolators(const float* clock_rate){
    for(auto& item:interpolators)if(item.duration!=0){
        Instruction origin{manager->loader->instruction(item.subroutine,item.instruction_offset)};
        ...
        auto& target=offset<0?*manager->float_destination(truncate_float(offset)):stack.absolute(item.frame_base+truncate_float(offset));
        target=float_bits(value);
    }
}
std::int32_t EnemyRuntime::tick(float delta,EnemyVmEnvironment& environment) {
    auto* random=environment.random;const auto* clock_rate=environment.clock_rate;
    if(!active())return -1;
    auto ins=current();
    for(;;) {
        ...
        if((ins.rank()&rank)!=0) {
            ...
            case 15:case 16:{
                ...
                manager->spawn(*this,ins,id,op==16?1:0);break;
            }
            case 17:case 18:case 19:case 20:{
                
                auto* target=manager->find_runtime(integer_argument(ins,0));
                if(target){if(op==17)target->instruction_offset=-1;else if(op==18)target->flags|=1;else if(op==19)target->flags&=~1u;else target->field_2c=integer_argument(ins,1);}
                drop=true;break;
            }
            case 21:manager->terminate_async();break;
            ...
            default:
                {
                    const auto result=manager->execute_opcode();
                    if(result==-1){tick_interpolators(clock_rate);return 0;}
    ...
    tick_interpolators(clock_rate);
    return 0;
}
EnemyRuntime& ScriptManager::spawn(EnemyRuntime& caller,Instruction instruction,std::int32_t id,std::int32_t skip){
    void* storage=runtime::allocate_bytes(sizeof(EnemyRuntime));if(!storage)throw std::bad_alloc();
    auto* task=new(storage)EnemyRuntime;task->manager=this;task->time=0;task->instruction_offset=task->subroutine=-1;task->async_id=id;task->rank=current->rank;
    auto* link=static_cast<scheduler::Link*>(runtime::allocate_bytes(sizeof(scheduler::Link)));if(!link)throw std::bad_alloc();
    scheduler::initialize_link(*link,reinterpret_cast<scheduler::Node*>(task));scheduler::insert_after(runtimes,*link);
    caller.call_into(instruction,*task,skip);return *task;
}
EnemyRuntime* ScriptManager::find_runtime(std::int32_t id){
    for(auto* link=&runtimes;link;link=link->next){auto* task=reinterpret_cast<EnemyRuntime*>(link->value);if(task->async_id==id)return task;}return nullptr;
}
void ScriptManager::terminate_async(){
    for(auto* link=runtimes.next;link;link=link->next){auto* task=reinterpret_cast<EnemyRuntime*>(link->value);task->instruction_offset=task->subroutine=-1;}
}
std::int32_t ScriptManager::tick(float delta,EnemyVmEnvironment& environment){
    auto* link=&runtimes;bool first=true;
    while(link){auto* next=link->next;current=reinterpret_cast<EnemyRuntime*>(link->value);const auto result=current->tick(delta,environment);
        if(first){if(result!=0)return -1;first=false;}
        else if(result!=0){current->~EnemyRuntime();runtime::release_bytes(current);scheduler::unlink(*link);runtime::release_bytes(link);}
        link=next;
    }
